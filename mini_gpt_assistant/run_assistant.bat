...
REM Check if config.py has distilgpt2 or another valid model name
findstr /C:"MODEL_NAME = \"distilgpt2\"" config.py >nul
if errorlevel 1 (
    REM Check if it's still PLACEHOLDER
    ...
    if not errorlevel 1 (
        ...
        echo Model configured! Checking if it's a valid model...
    )
) else (
    echo ✅ Default model (distilgpt2) detected!
)
...
if errorlevel 1 (
    ...
    echo.
    echo 5. To test your installation:
    echo    - Run: python demo.py (tests everything without starting chat)
    echo.
